<div>
    <form class="p-5 form-create" wire:submit="update">
        ...
        @if (!empty($existingImages))
            <div class="mb-3">
                <label class="form-label fw-bold">{{ __('ui.immaginiCaricateInPrecedenza') }}:</label>
                    ...
                    @foreach ($existingImages as $image)
                        ...
                        <div style="position: relative; width: 150px; height: 150px;">
                            ...
                            <button type="button" wire:click="removeExistingImage({{ $image['id'] }})"
                                class="btn btn-sm btn-danger position-absolute top-0 end-0"><i class="fa-regular fa-2x fa-trash-can"></i></button>
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        @if (!empty($temporary_images_final))
            <div class="mb-3">
                <label class="form-label fw-bold">{{ __('ui.anteprimaNuoveImmaginiCaricate') }}:</label>
                ...
                <div class="d-flex gap-3 flex-wrap border border-success rounded p-3">
                    @foreach ($temporary_images_final as $key => $image)
                        ...
                        <div style="position: relative; width: 150px; height: 150px;">
                            ...
                            <button type="button" class="btn btn-sm btn-danger position-absolute top-0 end-0" wire:click="removeTemporaryImage({{ $key }})">
